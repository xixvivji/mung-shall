...
type RoadmapDetail = {
  title: string;
  adopterTodos: string[];   // 체크박스로 처리할 “입양자 할 일”
  medicalInfo: string[];    // 의료/접종 정보(기관 확인은 다음 단계에서 상태 추가)
};
...
function validatePhoto(file: File): string | null {
  ...
  const okExt = [".jpg", ".jpeg", ".png"].some((ext) => lowerName.endsWith(ext));
...
export function CareStep() {
  ...
  const [day0Photo, setDay0Photo] = useState<{ file: File | null; error?: string }>({
    file: null,
  });
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <section className="rounded-xl border border-gray-200 p-4">
              ...
              {/* 기관 확인/승인(PDF 업로드) UI는 다음 단계에서 role 판별 코드 받은 후 붙임 */}
              <p className="mt-3 text-xs text-gray-400">
                * 일부 항목은 기관 확인이 필요합니다. (PDF 업로드/승인 기능은 다음 단계에서 추가)
              </p>
            </section>
          </div>
        </div>
